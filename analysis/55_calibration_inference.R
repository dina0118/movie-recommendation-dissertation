...
#
# The thesis's primary criterion is calibration, and the statistic used
# to measure it cannot carry the inferences that have been drawn from
# it. This file establishes the problem, quantifies it, and replaces
# the statistic with three measures that behave properly.
#
# RUN INSIDE THE SAME CLEAN 06b SESSION, AFTER 52 AND 54.
#
# ---------------------------------------------------------------------
# THE PROBLEM, AS 54 EXPOSED IT
#
# 54 produced two rows that cannot both be right:
#
#   M2d -> M2d+T   calibration -0.00389 [-0.00725, -0.00107]  excludes 0
#   M2d -> M2e     calibration -0.01379 [-0.01677, +0.00051]  spans 0
#
# The second effect is three and a half times the first and is the less
# significant of the two. For a well-behaved statistic that is not
# possible.
#
# The cause is in the definition. The chapter's calibration error is
#
#     calib_err = mean_k | mean(CP_k) - mean(IND_k) |
#
# an average of ABSOLUTE deviations. Absolute values do not commute
# with expectation: for any quantity whose true value is near zero,
# E|X_hat| > |E X|, and resampling can only push the folded statistic
# up, never down. So the better calibrated a model is, the more its
# bootstrap distribution is inflated relative to its point estimate,
# and the harder it becomes to distinguish from a worse model. The
# levels table in 54 shows exactly this asymmetry: M2e's interval runs
# [0.00299, 0.02140] around a point estimate of 0.00618, while M2d's
# runs [0.00826, 0.03642] around 0.01998.
#
# THIS IS NOT LOCAL TO 54. Section 7d's published table uses the same
# statistic and the same bootstrap, including
#   M3a  calibration -0.00118 [-0.00168, +0.00101]
# whose "cannot be distinguished from zero" verdict is partly a
# property of the folding rather than of the evidence. Every
# calibration interval in the thesis needs re-deriving, which is why
# the M3 fits are carried through this file alongside the ladder.
#
# ---------------------------------------------------------------------
# WHAT REPLACES IT
#
# (1) A NOISE FLOOR. Calibration error has no natural zero: even a
#     perfectly calibrated model shows a positive value on finite data,
#     and how positive depends on the sample and on the predictions
#     themselves. Section 2 simulates outcomes FROM EACH MODEL'S OWN
#     predicted distributions, so the simulated data is perfectly
#     calibrated for that model by construction, and reads off the
#     calibration error such a world produces. The question "is this
#     model distinguishable from perfectly calibrated?" then has an
#     answer. Section 9's own standing rule -- effect size is observed
#     minus null -- has never been applied to the primary metric.
#
# (2) SIGNED COMPONENTS. The four per-threshold deviations
#     d_k = mean(CP_k) - mean(IND_k) are means, so they bootstrap
#     normally and their contrasts are unproblematic. They also carry
#     the direction, which the folded statistic discards: a model that
#     is too pessimistic at the bottom of the scale and too optimistic
#     at the top has two large deviations that the average hides.
#
# (3) WEAK CALIBRATION. Cox intercept and slope, per threshold, from a
#     logistic regression of the outcome on the model's own logit.
#     Perfect calibration is intercept 0 and slope 1; a slope below 1
#     means the predictions are too extreme. This is the standard
#     apparatus in the reference already cited for calibration levels
#     (Van Calster et al. 2016) and is the natural home for the
#     sharpness-versus-location distinction the thesis argues about.
#
# Section 5 adds the paired test 54 section 3 was missing: the two
# spacing correlations are computed on the same users and are
# dependent, so their difference needs an interval rather than a
